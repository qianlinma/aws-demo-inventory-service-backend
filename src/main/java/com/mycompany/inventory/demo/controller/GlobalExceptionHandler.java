...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleDatabaseFailure(SdkException e) {
        logger.warn("Inventory database unavailable", e);
        return ResponseEntity
                .status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(new ApiErrorResponse("SERVICE_UNAVAILABLE", "Inventory service unavailable"));
    }

    @ExceptionHandler(RuntimeException.class)
    public ResponseEntity<ApiErrorResponse> handleUnexpectedFailure(RuntimeException e) {
        logger.error("Unexpected inventory service failure", e);
        return ResponseEntity
